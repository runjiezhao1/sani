...
void Sampler::generateSamples(int& numSamples, SampleMode sampleMode, WarpMethod warpMethod)
{
    ...
    for(int i = 0; i < numSamples; ++i)
    {
        int y = i / sqrtVal;
        int x = i % sqrtVal;
        //std::cout << "x " << x << " y " << y << " sqrt " << sqrtVal << std::endl;
        glm::vec2 sample;
        ...
        case GRID:
            //TODO
            sample = glm::vec2((x+0.5) / (float)sqrtVal, (y+0.5) / (float)sqrtVal);
            //throw std::runtime_error("You haven't yet implemented grid sampling!");
            break;

        case STRATIFIED:
            //TODO
            int other = i + rng.nextUInt(numSamples - i);
            float dx = 1 / (float)sqrtVal, dy = 1 / (float)sqrtVal;
            float jx = rng.nextFloat();
            float jy = rng.nextFloat();
            sample = glm::vec2(std::min((x + jx)*dx,OneMinusEpsilon),std::min((y + jy)*dy,OneMinusEpsilon));
